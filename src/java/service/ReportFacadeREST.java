...
import static com.health.Report_.username;
...
//*********************************Generate Report**************************
//    @GET
//    @Path("generateReport/{username}/{date}/{totalSteps}/{clorieSetGoal}")
//    @Produces({"application/json"})
//    public Report generateReport(@PathParam("userid") String uername,@PathParam("date") String date, @PathParam("totalSteps") int totalSteps,@PathParam("clorieSetGoal") double calorieSetGoal) throws ParseException {
...
//        double restCalorieBurned = Double.parseDouble(calculateTotalDailyCalorieBurned(username));
//        double stepCalorieBurned = Double.parseDouble(this.calculateCaloriesPerStepsTaken(username))*totalSteps;
...
//    @GET
//    @Path("calculateCaloriesPerStepsTaken/{username}")
//    @Produces({"application/json"})
//    public String calculateCaloriesPerStepsTaken(@PathParam("username") String username){
...
//    }
    ...
    @GET
    @Path("calculateCaloriesTotalStepsTaken/{username}/{date}")
    @Produces({"application/json"})
    public String calculateCaloriesTotalStepsTaken(@PathParam("username") String username,@PathParam("date") String date) throws ParseException{
//        CaloriesPerStepsTaken(username);
//        TypedQuery<Report> q = em.createQuery("SELECT c FROM FoodConsumed c WHERE c.date like " + "'%" + date + "%'", Report.class);
        ...
        Report r = (Report)query.getResultList().get(0);
        System.out.println("r.getTotalSteps");
        Double result = r.getTotalSteps() * Double.parseDouble(CaloriesPerStepsTaken(username));
    ...
    public String CaloriesPerStepsTaken( String username){
        DecimalFormat df = new DecimalFormat("0.0000");
        Query query = em.createNamedQuery("User.findByUsername");
        query.setParameter("username",username);
        User user = (User)query.getSingleResult();
...
//   @GET
//   @Path("calculateCalorieBurnedAndConsumeOnPeriod/{username}/{begindate}/{enddate}")
//   @Produces({"application/json"})
//   public String calculateCalorieBurnedAndConsumeOnPeriod(@PathParam("username") String username,@PathParam("begindate") String begindate,@PathParam("enddate") String enddate) throws ParseException{
//       DecimalFormat df = new DecimalFormat("0.0000");
//       TypedQuery<Report> q = em.createQuery("SELECT r FROM Report r WHERE r.username ="+username+" AND r.date between '"+begindate+"' AND '"+enddate+"'",Report.class);
...
//   }
   ...
   @GET
   @Path("findByPeriodAndusername/{username}/{begindate}/{enddate}")
   @Produces({"application/json"})
   public List<Report> findByPeriodAndusername(@PathParam("username") String username,@PathParam("begindate") String begindate,@PathParam("enddate") String enddate) throws ParseException{
      TypedQuery<Report> q = em.createQuery("SELECT r FROM Report r WHERE r.username ="+username+" "
                              + "AND r.date between '"+begindate+"' AND '"+enddate+"'",Report.class);
