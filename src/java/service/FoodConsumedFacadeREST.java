...
public class FoodConsumedFacadeREST extends AbstractFacade<FoodConsumed> {
 ...
 //**************************************Adding a Food********************************************************************** 
    ...
    @GET
    @Path("AlterFoodCount/{username}/{foodname}/{date}/{count}")
    @Produces({"application/json"})
    public String AlterFoodCount(@PathParam("count") int count,@PathParam("username") String username,@PathParam("foodname") String foodname,
...
//    } 
    ...
    @GET
    @Path("AddFoodCount/{username}/{foodname}/{count}")
    @Produces({"application/json"})
    public String AddFoodCount(@PathParam("count") int count,@PathParam("username") String username,
            @PathParam("foodname") String foodname     ){
         User user = getUserByUserName(username);
         Food food = getFoodByFoodName(foodname);
         FoodConsumed newConsumed = new FoodConsumed();
         newConsumed.setUserid(user);
         newConsumed.setFoodid(food);
         newConsumed.setCount(count);
         newConsumed.setDate(new Date());
         super.create(newConsumed);
         return "1";
     ...
     public Food getFoodByFoodName(String FoodName){
        ...
        Food f = (Food)query.getResultList().get(0);
        return f;
    }
}